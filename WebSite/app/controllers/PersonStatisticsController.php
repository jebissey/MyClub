...
{
    public function showStatistics($personId, $seasonStart = null, $seasonEnd = null)
    {
        // Si les dates de saison ne sont pas spécifiées, utiliser la saison courante
        if ($seasonStart === null || $seasonEnd === null) {
            $currentYear = date('Y');
            $currentMonth = date('m');
            
            // Si nous sommes avant septembre, la saison a commencé l'année précédente
            if ($currentMonth < 9) {
        ...
        $stats = [
            ...
            'seasonEnd' => $seasonEnd,
            'articles' => $this->getArticleStats($personId, $seasonStart, $seasonEnd),
            'surveys' => $this->getSurveyStats($personId, $seasonStart, $seasonEnd),
            'surveyReplies' => $this->getSurveyRepliesStats($personId, $seasonStart, $seasonEnd),
            'designs' => $this->getDesignStats($personId, $seasonStart, $seasonEnd),
            'designVotes' => $this->getDesignVoteStats($personId, $seasonStart, $seasonEnd),
            'events' => $this->getEventStats($personId, $seasonStart, $seasonEnd),
            'eventParticipations' => $this->getEventParticipationStats($personId, $seasonStart, $seasonEnd),
            'participantSupplies' => $this->getParticipantSupplyStats($personId, $seasonStart, $seasonEnd),
        ];

        // Récupérer toutes les saisons disponibles pour le filtre
        $seasons = $this->getAvailableSeasons();
        
        // Rendre la vue
        $this->latte->render('statistics/person.latte', [
            'stats' => $stats,
            'seasons' => $seasons,
            'currentSeason' => [
                'start' => $seasonStart,
    ...
    /**
     * Récupère les statistiques sur les articles
     */
    private function getArticleStats($personId, $seasonStart, $seasonEnd)
    {
        // Articles créés par la personne
        $query = "
    ...
    /**
     * Récupère les statistiques sur les sondages
     */
    private function getSurveyStats($personId, $seasonStart, $seasonEnd)
    {
        // Sondages créés par la personne (via les articles)
        $query = "
    ...
    /**
     * Récupère les statistiques sur les réponses aux sondages
     */
    private function getSurveyRepliesStats($personId, $seasonStart, $seasonEnd)
    {
        // Réponses aux sondages par la personne
        $query = "
            SELECT COUNT(*) as count 
            FROM Reply 
            WHERE IdPerson = ? 
        ...
        // Total des réponses aux sondages
        $query = "
            SELECT COUNT(*) as count 
            FROM Reply
            WHERE Id IN (
    ...
    /**
     * Récupère les statistiques sur les designs
     */
    private function getDesignStats($personId, $seasonStart, $seasonEnd)
    {
        // Designs créés par la personne
        $query = "
            SELECT COUNT(*) as count 
            FROM Design 
            WHERE IdPerson = ? 
    ...
    /**
     * Récupère les statistiques sur les votes de designs
     */
    private function getDesignVoteStats($personId, $seasonStart, $seasonEnd)
    {
        // Votes sur les designs par la personne
        $query = "
            SELECT COUNT(*) as count 
            FROM DesignVote 
            WHERE IdPerson = ? 
        ...
        // Total des votes sur les designs
        $query = "
            SELECT COUNT(*) as count 
            FROM DesignVote
            WHERE Id IN (
    ...
    /**
     * Récupère les statistiques sur les événements créés par type
     */
    private function getEventStats($personId, $seasonStart, $seasonEnd)
    {
        ...
        foreach ($eventTypes as $eventType) {
            // Événements créés par la personne pour ce type
            $query = "
                SELECT COUNT(*) as count 
                FROM Event 
                WHERE CreatedBy = ? 
            ...
            // Total des événements de ce type
            $query = "
                SELECT COUNT(*) as count 
                FROM Event 
                WHERE IdEventType = ?
    ...
    /**
     * Récupère les statistiques sur les participations aux événements par type
     */
    private function getEventParticipationStats($personId, $seasonStart, $seasonEnd)
    {
        ...
        foreach ($eventTypes as $eventType) {
            // Participations de la personne pour ce type d'événement
            $query = "
                SELECT COUNT(*) as count 
                FROM Participant p
            ...
            // Total des participations pour ce type d'événement
            $query = "
                SELECT COUNT(*) as count 
                FROM Participant p
                JOIN Event e ON p.IdEvent = e.Id
    ...
    /**
     * Récupère les statistiques sur les contributions aux besoins d'événements
     */
    private function getParticipantSupplyStats($personId, $seasonStart, $seasonEnd)
    {
        // Contributions de la personne
        $query = "
            SELECT COUNT(*) as count 
    ...
    /**
     * Récupère toutes les saisons disponibles
     */
